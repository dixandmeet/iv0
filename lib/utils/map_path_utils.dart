...
class MapPathUtils {
  ...
  /// Polylignes marche + contexte ligne TC (mode guidage piéton).
  static List<Polyline> walkGuidancePolylines({
    required List<LatLng> walkPath,
    required List<LatLng> transitShape,
    required LatLng stopOnLine,
    required Color lineColor,
  }) {
    final polylines = <Polyline>[];
    final nearLine = segmentNear(
      transitShape,
      stopOnLine,
      beforeMeters: 280,
      afterMeters: 280,
    ...
    final path = connectTo(walkPath, stopOnLine);
    if (path.length >= 2) {
      polylines.add(
        Polyline(
          points: path,
  ...
  /// Polylignes ligne TC avec tronçon parcouru / restant.
  static List<Polyline> transitGuidancePolylines({
    required List<LatLng> shape,
    required LatLng focusStop,
    required Color lineColor,
    LatLng? vehicle,
    LatLng? clipStart,
    LatLng? clipEnd,
    double? contextMeters,
  }) {
    if (shape.length < 2) return const [];

    var visible = shape;
    if (clipStart != null && clipEnd != null) {
      visible = clipBetween(shape, clipStart, clipEnd);
    } else if (contextMeters != null) {
      visible = segmentNear(
        shape,
        focusStop,
        beforeMeters: contextMeters,
        afterMeters: contextMeters,
      );
    }
    if (visible.length < 2) return const [];

    final vehicleIdx = vehicle != null
        ? nearestIndex(visible, vehicle)
        : nearestIndex(visible, focusStop);
    final passed = slice(visible, 0, vehicleIdx);
    final ahead = slice(visible, vehicleIdx, visible.length - 1);
